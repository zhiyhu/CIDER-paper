...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, fig.width = 9, fig.keep = TRUE, fig.path = "plots/dendritic/cutoff_selection/Rmarkdown_")
...
system.time({
  ...
  for(itor in 1:5) {
    ...
    # downsampling
    select <- downsampling(metadata = metadata, n_size = n_size, include = TRUE, replace = TRUE, lower.cutoff = 15)
    ...
    for (i in 1:nrow(combinations)){
      ...
      logCPM <- cpm(dge[, idx], log = TRUE, prior.count = 3)
      fit <- lmFit(logCPM, design)
      group_fit <- contrasts.fit(fit, contrast_m)
...
idx1 <- paste0(0:4, "_Batch1")
idx2 <- paste0(0:4, "_Batch2")
```

```{r plot-heatmap-similarity-pvalue}
p_mat <- getProbability(dist_coef[idx1, idx2], combinations_all$similarity)
pheatmap(p_mat, main = "Empirical p-values",
         color = rev(colorRampPalette(c("white", col_vector[8]))(50)), 
...
df_plot <- data.frame(tSNE_1 = seu@reductions$tsne@cell.embeddings[,1],
                      ...
                      pvalue = seu$pvalue)
p1 <- ggplot(df_plot, aes(x = tSNE_1, y = tSNE_2, fill = as.factor(label))) + geom_point(col = "grey40", size = 2, shape = 21)  + scale_fill_manual(values = col_vector[1:10]) + theme_classic() + theme(legend.position = "top", axis.ticks.x = element_blank(),axis.ticks.y = element_blank(),axis.text = element_blank()) 
p2 <- ggplot(df_plot, aes(x = tSNE_1, y = tSNE_2, fill = similarity)) + geom_point(col = "grey40", size = 2, shape = 21)  +  scale_fill_viridis(option = "B") + theme_classic() + theme(legend.position = "top", axis.ticks.x = element_blank(),axis.ticks.y = element_blank(),axis.text = element_blank()) 
p3 <- ggplot(df_plot, aes(x = tSNE_1, y = tSNE_2, fill = pvalue)) + geom_point(col = "grey40", size = 2, shape = 21)  +  scale_fill_viridis(option = "B", direction = -1) +  
