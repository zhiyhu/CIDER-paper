...
runtime_tmp <- system.time({
  # seu_list <- initialClusteringSeurat_faster(seu, res = 0.3, dim = 20, 
...
for(i in 1:length(seu_list)){
  print(mclust::adjustedRandIndex(seu_list[[i]]$seurat_clusters, seu_list[[i]]$Group))
}
...
for(i in 1:length(seu_list)){
  print(mclust::adjustedRandIndex(seu_list[[i]]$inicluster, seu_list[[i]]$Group))
}
...
runtime_tmp <- system.time({
  ...
  df <- data.frame(g = metadata$label[select],
                   ...
                   p = metadata$patient[select], # patient
                   t = metadata$timepoint[select], # timepoint
  ...
  df_dist <- foreach(i = combinations$g1, j = combinations$g2, df = rep(list(df), n.iter), 
                     logCPM = rep(list(logCPM), n.iter),.combine = "rbind") %dopar% 
    {
      ...
      # design and contrast
      frml <- "~0+tmp+b"
      if(length(unique(df$d[idx])) > 1){
        frml <- paste0(frml, "+t")
      }
      frml <- as.formula(paste0(frml, "+ detrate"))
      design <- model.matrix(frml, data = df[idx,])
